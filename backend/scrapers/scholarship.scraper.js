...
async function scrapeListingPage(page) {
  return await page.evaluate(() => {
    ...
    return rows.map(row => {
      ...
      const amountText = row.querySelector('td:nth-child(3)')?.innerText?.trim() || '';
      const amountValue = amountText.startsWith('$')
        ? Number(amountText.replace(/[^0-9]/g, ''))
        : null;

      const deadline = row.querySelector('td:nth-child(4)')?.innerText?.replace('Due Date:', '').trim() || '';
...
async function scrapeDetailPage(browser, url) {
  ...
  try {
    ...
    const detail = await page.evaluate(() => {
      const description = document.querySelector('div > div > p')?.textContent?.trim() || '';
